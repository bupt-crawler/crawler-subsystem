...
class PortableDeviceSpider(scrapy.Spider):
    ...
    start_urls = ['http://159.226.153.63/collectdata/portabledevice.aspx']
    custom_settings = {
        'ITEM_PIPELINES': {
    ...
    def __init__(self, **kwargs):
        ...
        browser = webdriver.Firefox(firefox_options=options, executable_path=DRIVER_WINDOWS)
    ...
    def extractData(self, tag):
        ...
        item['projectId'] = tagTdList[5].string.strip()
        item['date'] = tagTdList[6].string.strip()
        item['longitude'] = tagTdList[7].string.strip()
        item['latitude'] = tagTdList[8].string.strip()
