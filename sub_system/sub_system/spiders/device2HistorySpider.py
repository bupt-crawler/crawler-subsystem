...
class Device2historyspiderSpider(scrapy.Spider):
    ...
    start_urls = ['http://gprs.sj2000.org/Web/TDDataManage.aspx?type=2&EquipmentID=2653']
    custom_settings = {
        'ITEM_PIPELINES': {
    ...
    def extractData(self):
        ...
        # 获取产流重量
        runoffWeightTagList = soup.find_all('div', class_='x-grid3-cell-inner x-grid3-col-DataJzHou4')
        for runoffWeightTag in runoffWeightTagList:
            runoffWeightlist.append(runoffWeightTag.string)
        # 获取径流密度
        runoffDensityTagList = soup.find_all('div', class_='x-grid3-cell-inner x-grid3-col-DataJzHou5')
        for runoffDensityTag in runoffDensityTagList:
            runoffDensitylist.append(runoffDensityTag.string)
        # 获取设备电压
        deviceVTagList = soup.find_all('div', class_='x-grid3-cell-inner x-grid3-col-DataJzHou6')
        for deviceVTag in deviceVTagList:
            deviceVlist.append(deviceVTag.string)
        # 获取故障码
        errorCodeTagList = soup.find_all('div', class_='x-grid3-cell-inner x-grid3-col-DataJzHou7')
        for errorCodeTag in errorCodeTagList:
            errorCodelist.append(errorCodeTag.string)
        itemList = []
        for index in range(len(idlist)):
            item = device2HistoryItem()
